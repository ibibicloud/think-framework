...
class Query
{
    ...
    public function setInc($field, $step = 1, $lazyTime = 0)
    {
        ...
        if (empty($condition)) {
            // 没有条件不做任何更新
            throw new Exception('没有任何数据需要更新');
        }
    ...
    protected function throwNotFound($options = [])
    {
        if (!empty($this->model)) {
            ...
            throw new ModelNotFoundException('模型数据不存在:' . $class, $class, $options);
        }
        $table = is_array($options['table']) ? key($options['table']) : $options['table'];
        throw new DataNotFoundException('表数据不存在:' . $table, $table, $options);
    }
