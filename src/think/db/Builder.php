...
abstract class Builder
{
    ...
    protected function parseData(Query $query, $data = [], $fields = [], $bind = [])
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if (false !== strpos($key, '->')) {
                ...
                if ($options['strict']) {
                    throw new Exception('数据表字段不存在:[' . $key . ']');
                }
            } elseif (is_null($val)) {
                $result[$item] = 'NULL';
                ...
                switch (strtoupper($val[0])) {
                    ...
                    case 'DEC':
                        ...
                        break;
                    case 'EXP':
                        throw new Exception('不支持的数据表达式:[' . $val[0] . ']');
                }
    ...
    public function buildWhere(Query $query, $where)
    {
        ...
        foreach ($where as $logic => $val) {
            ...
            foreach ($val as $value) {
                ...
                if (is_array($value)) {
                    if (key($value) !== 0) {
                        throw new Exception('查询表达式错误:' . var_export($value, true));
                    }
    ...
    protected function parseWhereItem(Query $query, $field, $val, $rule = '', $binds = [])
    {
        ...
        if (!isset($whereStr)) {
            throw new Exception('查询表达式错误:' . $exp);
        }
    ...
    protected function parseExists(Query $query, $key, $exp, $value, $field, $bindType)
    {
        ...
        if ($value instanceof \Closure) {
            ...
            throw new Exception('查询表达式错误:' . $value);
        }
    ...
    {
        if (is_array($value)) {
            throw new Exception('查询表达式错误:' . $exp . var_export($value, true));
        }
        ...
        foreach ($order as $key => $val) {
            if ($val instanceof Expression) {
                ...
                if (preg_match('/^[\w\.]+$/', $key)) {
                    ...
                    throw new Exception('排序表达式错误:' . $key);
                }
            }
        }
